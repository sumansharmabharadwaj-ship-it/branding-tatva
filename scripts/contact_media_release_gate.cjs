...
assert(films.length === 5, `Expected 5 distinct Contact-route films, found ${films.length}`);
assert(references.length === films.length, "The Contact route repeats a film source");
...
process.stdout.write(`Contact media release gate passed: ${films.length} distinct films, all at least 16 seconds.\n`);
