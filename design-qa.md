# Design QA: Contact gratitude bidirectional scrub · 2026-09-09

## Findings

- No actionable P0, P1, or P2 mismatch remains. The gratitude scene now borrows Parker's direction-aware scroll continuity while retaining Branding Tatva's established meadow composition, typography, palette, copy, and finite ending.
- [P3] A direct `#thanks` arrival can compress several acknowledgement beats while the anchor and spring settle. This is acceptable because the complete ledger remains readable and deliberate upward scrolling now replays the earlier responses.

## Target and evidence

- Source visual truth path: `https://heyparker.ai/?utm_source=chatgpt.com`, inspected in the cloud browser at its persistent Step 2 scroll state. The reference is used only for direction-aware sequence continuity.
- Protected visual source: the approved Branding Tatva gratitude chapter at `https://branding-tatva-git-august-8-isolated-suman22.vercel.app/contact#thanks`.
- Implementation screenshot path: cloud-browser capture of the rebuilt local `/contact?motion-qa=gratitude-final#thanks` route at its resolved state. The capture was inspected inline and was not persisted as a repository artifact.
- Full-view comparison evidence: the Parker source and rebuilt Branding Tatva implementation were emitted together in one browser comparison call. The comparison confirms that only the interaction principle moved across; the two products' visual languages remain distinct.
- Focused region comparison: no additional crop was required because this pass changes the scroll-to-state mapping only. The existing ledger geometry, shared baton bounds, response typography, and progress line styling are unchanged; their active indices and final widths were inspected directly across receiving, revisiting, and resolved states.
- Source pixels: `1348 × 926`; implementation pixels: `1348 × 926`.
- CSS viewport: `1363 × 936`; device pixel ratio: `1`. Browser captures exclude scrollbar pixels.
- State: full motion; acknowledgement 3 receiving, complete resolved, intentional upward revisit, forward return to rest, keyboard inspection, held selection during scroll, and reduced motion.

## Comparison history

| Pass | Severity | Visible finding | Fix and post-fix evidence |
| --- | --- | --- | --- |
| 1 | P2 | Acknowledgements accumulated correctly on the way down, but scrolling upward left the scene frozen in its completed response. The visitor could not scrub the existing baton and response back through the sequence. | Receipt history remains cumulative while a separate scroll focus follows the current threshold in both directions. Browser evidence moved from the resolved state back to acknowledgement 1 with the matching response and `revisiting` phase, then returned to the clean resolved state on forward scroll. |
| 2 | P2 | The motion spring can rebound below the fourth threshold after completion, which initially looked like an intentional reverse gesture and left acknowledgement 3 focused at rest. | Intent is now derived from raw scroll progress through a hysteresis window while the spring remains responsible for visual easing. The final frame settles with no baton, no sequence focus, and response phase `resolved`. |
| 3 | P2 | Browser focus can move the page slightly while Arrow-key navigation enters the ledger; closing inspection with Escape could therefore inherit a false revisit state. | Escape and explicit note closure normalize the settled state when the page is still inside the final resting window. Browser verification moved keyboard focus from time to curiosity, then Escape restored no baton, no active note, and the resolved response. |

## Required fidelity surfaces

- Fonts and typography: the existing display serif, supporting sans, optical italic response, weights, wrapping, line height, and tracked micro-labels are unchanged.
- Spacing and layout rhythm: the one-screen meadow frame, two-column balance, ledger dimensions, row heights, response block, CTA row, footer handoff, and chapter rail are unchanged.
- Colors and visual tokens: the existing ivory, soil, sandstone, translucent borders, and luminous meadow exposure remain intact. No Parker color, card, border, or shadow treatment was imported.
- Image quality and asset fidelity: the project-owned gratitude film and poster remain unchanged, with no new generated asset, placeholder, custom icon, or code-drawn illustration.
- Copy and content: all Branding Tatva gratitude wording, acknowledgement labels, response lines, booking route, field-notes route, and opening section remain unchanged.
- Interaction and accessibility: forward and reverse scroll share the same focus baton and response; received state never rewinds; click or touch style selection remains held during scrolling; Arrow keys, Home, End, Escape, progress semantics, and activation-only announcements remain intact.
- Responsive behavior: the existing narrow-screen component remains present at a measured `322 px` frame width with `322 px` document width and no horizontal overflow. No breakpoint or geometry rule changed.

## Verification

- Forward scroll: acknowledgement 3 entered `receiving` with exactly one baton inside `your candour` and the first three notes marked received.
- Completion: all four notes produced `complete="true"`, `settled="true"`, no sequence focus, no baton, and response phase `resolved`.
- Completion line: outer and inner progress surfaces both measured `261.375 px` with `transform: none`.
- Reverse scroll: the completed scene returned to acknowledgement 1 with its matching response while all four received states remained preserved; forward scroll restored the resolved frame.
- Keyboard: Enter opened acknowledgement 1, ArrowDown moved focus and the baton to acknowledgement 2, and Escape restored the resolved state.
- Held selection: the selected acknowledgement remained pressed and readable while the page moved from `3571` to `3771` scroll pixels.
- Reduced motion: direct `#thanks` arrival kept all acknowledgements manual, with no focus baton and no automatically visited note. Full motion was restored afterward.
- Browser console: no errors or warnings were reported by the final local Contact tab.
- Production build: passed with Next.js `15.5.25`; all 79 static pages generated and `/contact` typechecked successfully.

## Implementation checklist

- [x] Keep acknowledgement receipt cumulative.
- [x] Scrub the existing focus baton and response backward with intentional upward scroll.
- [x] Distinguish user direction from spring rebound.
- [x] Restore a clean final frame after forward scroll, Escape, and manual closure.
- [x] Verify desktop, narrow-screen overflow, keyboard, held selection, reduced motion, console health, and the production build.

## Follow-up polish

- No remaining P3 motion change is recommended without altering the approved pacing or adding scroll distance below the closing scene.
...
- Completion line: local QA measured the outer and inner progress surfaces at the same `261.375 px` width. A release-95 production check exposed a `0.952381` spring-rest edge case; release 98 production QA now measures both surfaces at `261.375 px` with no residual transform.
...
- Browser console: the final release-98 production pass reported no application-origin errors or warnings. One `chrome-extension://` metadata-bridge error was excluded because it does not originate from the site.
- Production build: passed with Next.js `15.5.25`; all 79 static pages generated and `/contact` typechecked successfully.
...
- Implementation route: `/about#about-founder-led`
- Desktop evidence: `/workspace/scratch/e56e36b0002e/about-audit-evidence/founder-led-desktop-1440x900.png` (`1440 × 900`)
- Mobile evidence: `/workspace/scratch/e56e36b0002e/about-audit-evidence/founder-led-mobile-390x844.png` (`390 × 844`)
- Responsive evidence: short desktop, tablet, narrow mobile, zoom equivalent, and reduced motion captures in `/workspace/scratch/e56e36b0002e/about-audit-evidence`
- State: four stage strategy record, final Application stage, static touch and reduced motion fallback

## Findings and resolution

- P1: the browser audit could not start because the Playwright version expected a missing Chromium build. Resolved with automatic discovery of the compatible cached headless browser and a same process local production server.
- P2: the final founder record could be captured before its lift and settle sequence completed. Resolved by waiting for the final clip animation before asserting or capturing the state.
- P2: the desktop continuity pill collided with footer labels while the consent reserve was visible. Resolved by removing the redundant pill in that constrained state.
- P2: the interactive founder sheet clipped at `1024 × 600`, and pointer coarse tablets received an unsuitable interaction model. Both now receive a complete two column static reading mode.
- P2: the adjacent diagnosis record clipped its first label and final detail. The record floor now contains all three routes with breathing room.
- No remaining actionable P0, P1, or P2 issue appears in the eight viewport audit.

## Required fidelity surfaces

- Typography: existing display and body families, uppercase record labels, and protected reading floors remain intact across all captures.
- Spacing: the desktop sheet, short viewport reading mode, touch ledger, chapter gutter, and consent aware footer remain visibly contained.
...
- Interaction: four semantic tabs remain available; `End` selects Application and focuses its tab; exactly one tab is selected; the final promise and services link render in animated and static modes.
- Mobile chapter navigation exposes eight chapters, restores focus on `Escape`, lands the selected chapter below the sticky header, and visibly focuses its heading.
- No horizontal overflow, application console error, or control collision was found.
...
- `node scripts/about_resolution_rendered_gate.cjs`: passed at `1440 × 900`, `1024 × 600`, `1023 × 768`, `720 × 450`, `390 × 844`, and `320 × 568`, plus desktop and mobile reduced motion
- Browser evidence manifest: `/workspace/scratch/e56e36b0002e/about-audit-evidence/results.json`
