...
function TatvaFilm() {
  ...
  const [activeIndex,setActiveIndex] = useState(0);
  const { scrollYProgress } = useScroll({ target:ref,offset:["start start","end end"] });
  useMotionValueEvent(scrollYProgress,"change",(value)=>{ if(reduced||compact)return; setActiveIndex(Math.min(elements.length-1,Math.floor(Math.min(.999,value)*elements.length))); });

  if(reduced||compact){
    return <section className="film-paper bg-[#F4EFE6] py-24 text-[#22231F] sm:py-32"><div className="mx-auto max-w-[94rem] px-6 sm:px-10"><div className="film-reveal max-w-4xl"><p className="text-[0.6rem] uppercase tracking-[.3em] text-[#9D6B4C]">The five Tatvas</p><h2 className="mt-5 font-display text-[clamp(3.3rem,7vw,6.8rem)] leading-[.9] tracking-[-.05em]">Five decisions. One remembered brand.</h2></div><div className="mt-12 space-y-6">{elements.map((element,index)=>{const change=tatvaTransformations[element.slug];return <article key={element.slug} className="film-reveal relative overflow-hidden rounded-[1.8rem] border border-[#22231F]/10 bg-[#EAE1D4]/70 p-6 sm:p-8"><div className="grid gap-8 sm:grid-cols-[.84fr_1.16fr] sm:items-center"><div><p className="text-[.54rem] uppercase tracking-[.22em]" style={{ color:element.color }}>Tatva {String(index+1).padStart(2,"0")} · {change.verb}</p><h3 className="mt-4 font-display text-5xl leading-none">{element.name.split(" · ")[0]}.</h3><p className="mt-5 text-sm leading-relaxed text-[#22231F]/58">{element.meaning}</p></div><div className="grid gap-5"><div><p className="text-[.52rem] uppercase tracking-[.18em] text-[#22231F]/34">Before</p><p className="mt-2 font-display text-2xl leading-tight text-[#22231F]/58">{change.before}</p></div><div><p className="text-[.52rem] uppercase tracking-[.18em]" style={{ color:element.color }}>After</p><p className="mt-2 font-display text-2xl leading-tight">{change.after}</p></div><p className="border-t border-[#22231F]/9 pt-4 text-xs leading-relaxed text-[#22231F]/48"><span className="text-[#22231F]/74">Without {element.name.split(" · ")[0]}:</span> {change.consequence}</p></div></div></article>})}</div></div></section>;
  }
  ...
  return <section ref={ref} className="relative h-[430svh] bg-[#1E2A22] text-[#22231F]"><div className="sticky top-0 h-svh min-h-[680px] overflow-hidden"><AnimatePresence mode="popLayout" initial={false}><motion.div key={active.slug} className="absolute inset-0" initial={{ opacity:.16,scale:1.08 }} animate={{ opacity:1,scale:1 }} exit={{ opacity:.14,scale:1.035 }} transition={{ duration:.82,ease:EASE }}><AmbientVideo src={active.video} poster={active.image} position={active.imagePosition??"center"}/></motion.div></AnimatePresence><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(244,239,230,.96)_0%,rgba(244,239,230,.88)_43%,rgba(244,239,230,.24)_72%,rgba(24,34,28,.48)_100%)]"/><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(244,239,230,.18),transparent_42%,rgba(24,34,28,.36))]"/>
      <div className="absolute inset-x-6 top-7 z-30 flex items-center justify-between text-[.52rem] uppercase tracking-[.22em] sm:inset-x-10 lg:inset-x-14"><span className="text-[#22231F]/46">The five Tatvas</span><span className="text-[#22231F]/46">{String(activeIndex+1).padStart(2,"0")} / 05</span></div>
      <div className="relative z-10 mx-auto flex h-full max-w-[94rem] items-center px-6 sm:px-10 lg:px-14"><div className="grid w-full gap-10 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:gap-16"><AnimatePresence mode="wait" initial={false}><motion.div key={`${active.slug}-copy`} initial={{ opacity:.22,y:36 }} animate={{ opacity:1,y:0 }} exit={{ opacity:.18,y:-24 }} transition={{ duration:.62,ease:EASE }}><p className="text-[.58rem] font-medium uppercase tracking-[.28em]" style={{ color:active.color }}>{change.verb}</p><h2 className="mt-5 font-display text-[clamp(4.7rem,10vw,10rem)] font-normal leading-[.78] tracking-[-.07em]">{active.name.split(" · ")[0].toLowerCase()}.</h2><p className="mt-8 max-w-xl font-display text-[clamp(2rem,3.7vw,3.9rem)] leading-[1.02]">{active.poetic}</p><p className="mt-6 max-w-xl text-sm leading-relaxed text-[#22231F]/58 sm:text-base">{active.meaning}</p></motion.div></AnimatePresence><div className="grid gap-7 lg:grid-cols-[1fr_.9fr] lg:items-center"><div className="relative h-[20rem] overflow-hidden rounded-[1.8rem] border border-[#22231F]/10 bg-[#F5F0E8]/66 p-5 backdrop-blur-xl sm:h-[25rem]"><AnimatePresence mode="wait" initial={false}><motion.div key={`${active.slug}-diagram`} className="h-full w-full" initial={{ opacity:.2,scale:.91 }} animate={{ opacity:1,scale:1 }} exit={{ opacity:.18,scale:1.06 }} transition={{ duration:.64,ease:EASE }}><TatvaDiagram slug={active.slug} color={active.color}/></motion.div></AnimatePresence></div><AnimatePresence mode="wait" initial={false}><motion.div key={`${active.slug}-change`} initial={{ opacity:.22,x:28 }} animate={{ opacity:1,x:0 }} exit={{ opacity:.18,x:-18 }} transition={{ duration:.52,ease:EASE }} className="rounded-[1.5rem] border border-[#22231F]/10 bg-[#F5F0E8]/72 p-5 backdrop-blur-xl sm:p-6"><div><p className="text-[.52rem] uppercase tracking-[.18em] text-[#22231F]/34">Before</p><p className="mt-3 font-display text-2xl leading-tight text-[#22231F]/58">{change.before}</p></div><div className="mt-6"><p className="text-[.52rem] uppercase tracking-[.18em]" style={{ color:active.color }}>After</p><p className="mt-3 font-display text-2xl leading-tight">{change.after}</p></div><p className="mt-6 border-t border-[#22231F]/9 pt-5 text-xs leading-relaxed text-[#22231F]/48"><span className="text-[#22231F]/74">Without {active.name.split(" · ")[0]}:</span> {change.consequence}</p></motion.div></AnimatePresence></div></div></div></div>
      <div className="absolute inset-x-6 bottom-6 z-30 sm:inset-x-10 lg:inset-x-14"><div className="mb-4 flex items-center justify-between gap-3">{elements.map((element,index)=><span key={element.slug} className={`text-[.48rem] uppercase tracking-[.13em] transition-opacity ${index===activeIndex?"opacity-100":"opacity-30"}`} style={{ color:index===activeIndex?element.color:PALETTE.ink }}>{String(index+1).padStart(2,"0")} {element.name.split(" · ")[0]}</span>)}</div><div className="h-px bg-[#22231F]/12"><motion.div className="h-full origin-left bg-[#9D6B4C]" style={{ scaleX:progress }}/></div></div></div></section>;
}
